#!/bin/bash
dir=`pwd`
flag=`echo $dir|awk '{print match($0,"h08/q04")}'`;
if [ $flag -gt 0 ]
then
	qdir=$dir
else
...
# Looking for correct count line
		...
		$script 1.2 1.3 1.5 2.0 2.5 3.0  > .out
...
# Looking for correct sum line
        	msg1="Sum value    "
        	v1=1					# Set expected value for test 1
        	n1=$( cat .out | grep -i sum | grep -iwE "11.5|=11.5|sum=11.5" | wc -l)
        	t1=$( [ $n1 -eq $v1 ] ; echo $? )	# Modify as needed
        	check $t1 $n1 "$msg1"
# Looking for correct ave line
        	msg1="Ave value    "
        	v1=1					# Set expected value for test 1
        	n1=$( cat .out | grep -i ave | grep -iwE "1.91|=1.91|min=1.91" | wc -l)
        	t1=$( [ $n1 -eq $v1 ] ; echo $? )	# Modify as needed
        	check $t1 $n1 "$msg1"
